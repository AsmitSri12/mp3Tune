...
async function convertToMp3(url, tempDir) {
  ...
  try {
    ...
    if (info.duration > MAX_DURATION) {
      throw new Error(
        `Video exceeds maximum duration of ${MAX_DURATION / 60} minutes.`,
      );
    }
    ...
    if (!fs.existsSync(finalFile)) {
      throw new Error(
        "Failed to create MP3 file. The file might be blocked or conversion failed.",
      );
    }
    ...
    if (
      ...
      errorMessage =
        "YouTube blocked the request. Please try again later or try a different video.";
    }
